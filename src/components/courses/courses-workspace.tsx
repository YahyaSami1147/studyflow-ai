...
export function CoursesWorkspace() {
  ...
  return <>
    ...
    <Modal open={formOpen} title={editingCourse ? `Update ${editingCourse.name}` : "Add a course"} onClose={() => setFormOpen(false)}><p className="-mt-4 mb-6 text-sm leading-6 text-[var(--muted)]">{editingCourse ? "Keep the details current so your workspace stays useful." : "Start with a subject you want to organize."}</p><CourseForm key={editingCourse?.id ?? "new"} initialCourse={editingCourse} onSubmit={handleSubmit} onCancel={() => setFormOpen(false)} /></Modal>
...
function EmptyCourses({ onAdd }: { onAdd: () => void }) { return <section className="mt-8 rounded-[var(--radius-card)] border border-dashed border-slate-300 bg-white px-6 py-16 text-center sm:px-10"><div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-blue-50 text-blue-600"><Plus size={22} aria-hidden="true" /></div><h2 className="mt-5 text-xl font-semibold tracking-tight text-slate-950">No courses yet</h2><p className="mx-auto mt-2 max-w-md text-sm leading-6 text-[var(--muted)]">Add your subjects to organize assignments and track your study progress.</p><button type="button" onClick={onAdd} className="mt-6 inline-flex h-10 items-center gap-2 rounded-md bg-blue-600 px-4 text-sm font-semibold text-white hover:bg-blue-700 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-600"><Plus size={16} aria-hidden="true" />Add your first course</button></section>; }
