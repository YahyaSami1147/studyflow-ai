...
export function StudyFlowChat() {
  ...
  return <section className="relative flex h-[calc(100dvh-13rem)] min-h-[28rem] flex-col overflow-hidden rounded-[var(--radius-card)] border border-[var(--line)] bg-slate-950/40 shadow-[0_12px_32px_rgba(2,6,23,0.35)]" aria-label="StudyFlow AI chat">
    ...
    <div className="border-t border-[var(--line)] bg-slate-950/60 p-3 sm:p-5"><form onSubmit={handleSubmit} className="mx-auto flex max-w-3xl flex-wrap items-end gap-2 rounded-lg border border-[var(--line)] bg-slate-900/60 p-2 shadow-sm focus-within:border-cyan-400"><label htmlFor="studyflow-message" className="sr-only">Message StudyFlow AI</label><textarea ref={composerRef} id="studyflow-message" value={input} onChange={(event) => setInput(event.target.value)} onKeyDown={handleKeyDown} rows={1} placeholder="Ask about your study plan, courses, or next task..." className="max-h-32 min-h-11 min-w-[8rem] flex-1 resize-none bg-transparent px-2 py-3 text-sm leading-5 text-white outline-none placeholder:text-slate-400" /><div className="flex shrink-0 flex-wrap justify-end gap-2">
      ...
      {isGenerating && <button type="button" onClick={() => void stop()} className="flex h-11 shrink-0 items-center gap-2 rounded-md bg-slate-800 px-3 text-sm font-semibold text-white transition-colors hover:bg-slate-700" aria-label="Stop generating"><Square size={14} fill="currentColor" aria-hidden="true" /><span>Stop</span></button>}
    </div></form><p className="mx-auto mt-2 max-w-3xl px-2 text-[11px] text-slate-300">Enter to send · Shift+Enter for a new line</p></div>
  </section>;
}

function EmptyState({ onChoosePrompt }: { onChoosePrompt: (prompt: string) => void }) {
  return <div className="mx-auto flex min-h-[22rem] w-full max-w-2xl flex-col items-center justify-center text-center"><span className="flex h-14 w-14 items-center justify-center rounded-full bg-cyan-500/10 text-cyan-300"><Bot size={26} strokeWidth={1.7} aria-hidden="true" /></span><h2 className="mt-5 text-xl font-semibold tracking-tight text-white">What would you like to study today?</h2><p className="mt-2 max-w-md text-sm leading-6 text-slate-300">Ask for an explanation, a focused plan, or a quick way to test what you know.</p><div className="mt-6 flex w-full flex-col gap-2 sm:flex-row sm:flex-wrap sm:justify-center">{EXAMPLE_PROMPTS.map((prompt) => <button key={prompt} type="button" onClick={() => onChoosePrompt(prompt)} className="rounded-lg border border-[var(--line)] bg-slate-900/60 px-3 py-2 text-left text-sm font-medium text-white transition-colors hover:bg-slate-800 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-cyan-300 sm:text-center">{prompt}</button>)}</div></div>;
}
