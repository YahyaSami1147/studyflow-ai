...
export function PlaceholderPage({ title, description, eyebrow = "Workspace" }: { title: string; description: string; eyebrow?: string }) {
  return (
    <AppShell>
      ...
      <section className="mt-8 rounded-[var(--radius-card)] border border-dashed border-slate-300 bg-white px-6 py-16 text-center sm:px-10" aria-label={`${title} empty state`}>
        ...
        <p className="mx-auto mt-2 max-w-sm text-sm leading-6 text-[var(--muted)]">Your {title.toLowerCase()} workspace will appear here when the core build is ready.</p>
      </section>
    </AppShell>
  );
}
